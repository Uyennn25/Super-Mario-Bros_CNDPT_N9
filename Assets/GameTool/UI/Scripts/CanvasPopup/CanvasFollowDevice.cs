...
#pragma warning restore CS0414
        ...
        private void FixCamSizeFollowScreen()
        {
            ...
            for (int i = 0; i < Resolutions.Count; i++)
            {
                ...
                {
                    if (cam && changeCamSize)
                    {
                        if (cam.orthographic)
                        {
                            cam.orthographicSize = Resolutions[i].CamSize;
                        }
                        else
                        {
                            cam.fieldOfView = Resolutions[i].PerspectiveSize;
                        }
                    }
                ...
                else
                {
                    ...
                    if (AspectCam > Resolutions[i].AspectCam && AspectCam < Resolutions[maxIndex].AspectCam)
                    {
                        if (cam && changeCamSize)
                        {
                            if (cam.orthographic)
                            {
                                //cam.orthographicSize = Resolutions[maxIndex].CamSize;
                                cam.orthographicSize = Resolutions[i].CamSize + (AspectCam - Resolutions[i].AspectCam) /
                                    (Resolutions[maxIndex].AspectCam - Resolutions[i].AspectCam) *
                                    (Resolutions[maxIndex].CamSize - Resolutions[i].CamSize);
                            }
                            else
                            {
                                cam.fieldOfView = Resolutions[i].PerspectiveSize + (Aspect - Resolutions[i].AspectCam) /
                                    (Resolutions[maxIndex].AspectCam - Resolutions[i].AspectCam) *
                                    (Resolutions[maxIndex].PerspectiveSize - Resolutions[i].PerspectiveSize);
                            }
                        }
            ...
            {
                this.PostEvent(EventID.ChangeCamera);
            }
        }
    }
