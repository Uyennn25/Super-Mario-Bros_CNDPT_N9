...
namespace GameTool.UI.Scripts.CanvasPopup
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        [SerializeField] bool Logging; // Conform to screen safe area on Y-axis (default true, disable to ignore)
        ...
        void Awake()
        {
            ...
            this.RegisterListener(EventID.ChangeCamera, OnChangeCamera);
        }

        private void OnDisable()
        {
            this.RemoveListener(EventID.ChangeCamera, OnChangeCamera);
        }

        private Coroutine _routineOnChangeCamera;
        private void OnChangeCamera(Component arg1, object[] arg2)
        {
            if (_routineOnChangeCamera != null)
            {
                StopCoroutine(_routineOnChangeCamera);
            }
            _routineOnChangeCamera = StartCoroutine(WaitSafeArea());

            IEnumerator WaitSafeArea()
            {
                yield return null;
                yield return null;
        ...
        void Refresh()
        {
            ...
            this.PostEvent(EventID.SafeAreaChange);
        }
